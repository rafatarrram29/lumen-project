...
function CustomTooltip({
  ...
  label,
  t,
}: TooltipContentProps<ValueType, NameType> & { t: Translations }) {
    ...
    <div className="rounded-lg border border-bdr bg-surf2 px-3 py-2 text-xs shadow-lg">
      ...
      {row?.areaValue !== null && row?.areaValue !== undefined && (
        <div className="flex items-center gap-1.5 text-amber">
          ...
          {t.chart.thisArea} {row.areaIndex}
          {` ${t.chart.idx} · `}
      ...
      {row?.clusterValue !== null && row?.clusterValue !== undefined && (
        <div className="mt-1 flex items-center gap-1.5 text-cyan">
          ...
          {t.chart.clusterAvg} {row.clusterIndex}
          {` ${t.chart.idx} · `}
...
export function TrendChart({
  ...
  return (
    <div>
      <div className="mb-1 text-xs text-muted">{t.chart.indexedCaption(areaLabel)}</div>
