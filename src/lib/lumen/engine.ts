...
function groupAreaMonthTotals(records: SalesRecord[]): Map<string, Map<number, number>> {
...
export function buildReport(records: SalesRecord[], year: number): Report {
  ...
  for (const [area, months] of areaTotals) {
    ...
    const change = pctChange(prevVal, currVal);
    const trendVals = lastN.filter((m) => months.has(m)).map((m) => months.get(m)!);
    const decliningStreak =
      trendVals.length >= 2 &&
      trendVals.every((v, i) => i === trendVals.length - 1 || v > trendVals[i + 1]);

    areaChanges[area] = {
      prevValue: Math.round(prevVal),
      currValue: Math.round(currVal),
      pctChange: change,
      decliningStreak,
  ...
  if (isSystemicDrop) {
    ...
    const worstEntry = Object.entries(famChanges).reduce((best, entry) =>
    ...
    for (const area of droppingAreas) {
      ...
      const famChanges: Record<string, { pctChange: number | null; absDrop: number }> = {};
      for (const [fam, months] of famData) {
        if (months.has(latest) && months.has(prev)) {
          famChanges[fam] = {
            pctChange: pctChange(months.get(prev)!, months.get(latest)!),
            absDrop: Math.round(months.get(prev)! - months.get(latest)!),
          };
        }
      }
      if (Object.keys(famChanges).length === 0) continue;
